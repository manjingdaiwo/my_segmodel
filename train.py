...
def get_args():
    ...
    arg("-c", "--config_path", type=Path, help="select the config path", required=True)
...
if args.resume:
    ...
    print("**************resumed model have loaded successfully****************")
...
def train(train_loader, net, criterion, optimizer, val_loader, lr_sch):
    ...
    while True:
        ...
        for i, data in loop:
            imgs, imgs1, labels = data['img'], data['img1'], data['gt_semantic_seg']   #igms:rgb, imgs1:dsm
            if config.gpu:
                 imgs = imgs.cuda().float()
                 imgs1 = imgs1.cuda().float()
                 labels = labels.cuda().long()
            outputs = net(imgs, imgs1)     
...
def validate(val_loader, net, criterion):
    ...
    for _, data in enumerate(val_loader):
        ...
        imgs, imgs1, labels = data['img'], data['img1'], data['gt_semantic_seg']

        if config.gpu:
            imgs = imgs.cuda().float()
            imgs1 = imgs.cuda().float()
            labels = labels.cuda().long()
